...
button_text_list = ['算了吧', '不行', '不可能', '呵', '不不不']


# 按钮
def button(text, x, y, w, h, color, screen):
    pygame.draw.rect(screen, color, (x, y, w, h))
    font = pygame.font.Font('./images/simkai.ttf', 20)
...
def title(text, screen, scale, color=(0, 0, 0)):
    font = pygame.font.Font('./images/simkai.ttf', WIDTH // (len(text) * 2))
...
def show_like_interface(text, screen, color=(255, 0, 0)):
    ...
    # 图片位置
    imgRect.midtop = 320, 0
    screen.blit(img, imgRect)
    font = pygame.font.Font('./images/simkai.ttf', WIDTH // (len(text)*2))
    textRender = font.render(text, True, color)
    textRect = textRender.get_rect()
    textRect.midtop = (WIDTH / 2, HEIGHT / 1.5)
    screen.blit(textRender, textRect)
...
def main():
    ...
    pygame.display.set_caption('xxx小姐姐专属')
    ...
    while running:
        ...
        # 加载图片
        img = pygame.image.load("./images/3.jpg")
        imgRect = img.get_rect()
        # 图片位置
        ...
        if mouse_pos[0] < unlike_pos_x + unlike_pos_width and mouse_pos[0] > unlike_pos_x and \
            ...
            while True:
                ...
                if mouse_pos[0] < unlike_pos_x + unlike_pos_width and mouse_pos[0] > unlike_pos_x and \
                    ...
                    continue
                break
        title('xxx，我可喜欢你了', screen, scale=[1.8, 10])
        title('可以给亲一口吗', screen, scale=[1.8, 5])
        title('嘻嘻嘻', screen, scale=[1.8, 3])
        ...
        pygame.display.update()
        clock.tick(60)
    show_like_interface('小姐姐最好了', screen, color=(0, 0, 7))
